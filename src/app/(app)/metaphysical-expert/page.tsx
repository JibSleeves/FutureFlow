...
import { BrainCog, User, Send } from "lucide-react";
...
export default function MetaphysicalExpertPageClient() {
  ...
  const handleSubmit = () => {
    if (!userInput.trim()) {
      toast({ variant: "destructive", title: "Empty Message", description: "Please type your question or topic." });
      return;
    }
    setError(null);
    ...
    startTransition(async () => {
      // Send only the last MAX_HISTORY_LENGTH messages for context
      const recentHistory = chatHistory.slice(-MAX_HISTORY_LENGTH + 1); // +1 because newUserMessage is not yet in chatHistory state for this call
      ...
      if ('error' in result) {
        ...
        // Optionally add the error as an AI message or allow resubmission
      } else {
    ...
    <div className="container mx-auto max-w-3xl space-y-8 pb-16 h-[calc(100vh-10rem)] flex flex-col">
      <header className="text-center">
        <h1 className="text-4xl font-bold tracking-tight text-primary flex items-center justify-center gap-3">
          <BrainCog className="h-10 w-10 text-accent" /> Meta-Physical Expert
        </h1>
        <p className="mt-2 text-lg text-muted-foreground">
          Explore the frontiers of understanding with an AI guide.
        </p>
      </header>

      <Card className="shadow-xl bg-card/50 backdrop-blur-sm flex-grow flex flex-col">
        <CardHeader>
          <CardTitle>Chat with the Expert</CardTitle>
          <CardDescription>
            Pose your questions, share your theories, and delve into the metaphysical.
          </CardDescription>
        </CardHeader>
                ...
                >
                  {msg.role === 'assistant' && (
                    <Avatar className="h-8 w-8 border border-accent/50">
                      <AvatarFallback><BrainCog className="h-5 w-5 text-accent" /></AvatarFallback>
                    </Avatar>
                  )}
                  <div
                    className={cn(
        ...
        <CardFooter className="border-t pt-4">
          ...
          >
            <Input
              id="user-input"
              placeholder="Ask about quantum physics, ancient wisdom, consciousness..."
              value={userInput}
