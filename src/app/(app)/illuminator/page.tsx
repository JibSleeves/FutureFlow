...
import { Eye, User, Send, Bot } from "lucide-react";
import { useToast } from "@/hooks/use-toast";
import { Alert, AlertDescription, AlertTitle } from "@/components/ui/alert";
import { LoadingSpinner } from '@/components/common/loading-spinner';
import type { IlluminatorChatInput, ChatMessage } from '@/ai/flows/illuminator-chat-flow';
import { handleIlluminatorChatAction } from './actions';
import { cn } from '@/lib/utils';

const MAX_HISTORY_LENGTH = 12;

export default function IlluminatorPageClient() {
  ...
  const handleSubmit = () => {
    if (!userInput.trim()) {
      toast({ variant: "destructive", title: "Question Unspoken to the Veil Lifter", description: "Present your inquiry, for hidden truths may be unveiled." });
      return;
    }
    setError(null);
    ...
    startTransition(async () => {
      ...
      const input: IlluminatorChatInput = {
        userMessage: currentInput,
        chatHistory: recentHistory.filter(msg => msg.role !== 'user' || msg.content !== currentInput),
      };
      
      const result = await handleIlluminatorChatAction(input);

      if ('error' in result) {
        setError(result.error);
        toast({ variant: "destructive", title: "Illuminator Veiled in Shadow", description: result.error });
      } else {
    ...
    <div className="container mx-auto max-w-4xl space-y-8 pb-16 h-[calc(100vh-6rem)] flex flex-col"> {/* Increased max-width */}
      <header className="text-center py-8">
        <h1 className="text-5xl md:text-6xl font-lora font-bold tracking-wider text-primary flex items-center justify-center gap-4 mb-3">
          <Eye className="h-10 w-10 md:h-12 md:w-12 text-accent animate-pulse-glow" /> Veil Lifter <Eye className="h-10 w-10 md:h-12 md:w-12 text-accent animate-pulse-glow" />
        </h1>
        <p className="mt-2 text-xl md:text-2xl text-muted-foreground font-serif italic text-flicker">
          Uncover hidden truths, explore esoteric orders, and theorize on clandestine organizations.
        </p>
      </header>
      ...
      <Card className="shadow-ornate bg-card/70 backdrop-blur-md border-2 border-primary/30 rounded-xl flex-grow flex flex-col overflow-hidden">
        <CardHeader className="bg-secondary/50 p-5 border-b-2 border-primary/40">
          <CardTitle className="font-lora text-primary text-2xl tracking-wider">Converse with the Illuminator</CardTitle>
          <CardDescription className="font-serif italic text-muted-foreground">
            Inquire about secret societies, esoteric orders, their veiled mysteries, and storied histories.
          </CardDescription>
        </CardHeader>
                ...
                >
                  {msg.role === 'assistant' && (
                    <Avatar className="h-10 w-10 border-2 border-accent/60 shadow-md bg-secondary/50">
                      <AvatarFallback className="bg-transparent"><Eye className="h-6 w-6 text-accent animate-pulse-glow" /></AvatarFallback>
                    </Avatar>
                  )}
                  <div
                    className={cn(
        ...
        <CardFooter className="border-t-2 border-primary/30 p-4 bg-secondary/40">
          ...
          >
            <Input
              ref={inputRef}
              id="user-input"
              placeholder="Ask of Rosicrucians, Templars, hidden lore, ancient conspiracies..."
              value={userInput}
      ...
      {error && !isPending && (
        <Alert variant="destructive" className="shadow-ornate mt-4">
          <Eye className="h-5 w-5 text-destructive-foreground/80" />
          <AlertTitle className="font-lora text-lg">Illumination Obscured by Doubt</AlertTitle>
