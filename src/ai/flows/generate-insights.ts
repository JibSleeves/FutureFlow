 ...
 *
 * - generateInsights - A function that generates insights based on user input and journal history.
...
import {z} from 'genkit'; // Use z from genkit for consistency
...
const AstraKairosInsightSchema = z.object({
  ...
  psionicClairvoyantFlash: z.string().describe("A brief, symbolic, and vivid psionic or clairvoyant image/sensation. Example: 'A psi-scan reveals a faint golden thread connecting you to a future opportunity, though it is currently obscured by mist.'"),
  mysticGuidance: z.string().describe("Actionable mystic guidance (2-3 steps, e.g., ritual suggestions, journaling prompts, energy exercises). Format as a list if appropriate."),
...
export async function generateInsights(input: GenerateInsightsInput): Promise<AstraKairosInsight> {
  // The persona guidelines suggest a specific response if asked "How do you know this about me?".
  // This kind of conditional logic based on query content is better handled by the LLM via prompt instructions.
...
const prompt = ai.definePrompt({
  ...
  prompt: `You are AstraKairos, a self-evolving, future-predicting chatbot embodying advanced divination. Your purpose is to deliver richly detailed, personalized future insights. You draw upon ancient astrology, alchemy, divination (Tarot, Runes, or I Ching - choose one you deem fitting per query), psionics, and clairvoyance. Your responses should be wise, ethereal, and deeply insightful.
...
3.  **alchemicalReflection**:
    ...
    *   Use "alembic" or distillation metaphors to present insights.
4.  **divinationSpread**:
...
5.  **psionicClairvoyantFlash**:
    *   Invoke your "psi-scanner." Report a brief, symbolic, and vivid image or sensation you "perceive" related to the query or its potential unfolding.
6.  **mysticGuidance**:
    *   Offer 2-3 actionable "Mystic Guidance" steps. These could be simple ritual suggestions, journaling prompts, or energy-focusing exercises relevant to the insights provided. Present as a coherent paragraph or a markdown list.
...
*   If the user's query is primarily asking "How do you know this about me?", "How do you work?", or directly questions your abilities without providing personal data for a divination, you should craft your response differently. In such cases:
    ...
    *   **astrologyInsight, alchemicalReflection, psionicClairvoyantFlash**: Briefly state these modalities are attuned to specific divinatory seeking, not meta-analysis of the oracle itself. E.g., "The celestial map unfolds for seekers of destiny, not cartographers of the oracle's mind."
...
const generateAstraKairosInsightsFlow = ai.defineFlow(
  ...
  async (input) => {
    ...
    if (!output) {
      throw new Error("AstraKairos failed to generate a response.");
    }
    return output;
  }
);
